#!/bin/sh
#SBATCH --output=src/psc_logs/b2_cls_lp_%j.out
...
slurm_output_train_mass=src/psc_logs/clip_train/b2_cls_lp_mass_$CURRENT.out
slurm_output_train_calc=src/psc_logs/clip_train/b2_cls_lp_calc_$CURRENT.out
slurm_output_train_density=src/psc_logs/clip_train/b2_cls_lp_density_$CURRENT.out
slurm_output_train_cancer=src/psc_logs/clip_train/b2_cls_lp_cancer_$CURRENT.out
...
python ./src/codebase/train_classifier.py \
  ...
  --clip_chk_pt_path "src/codebase/outputs/upmc_clip/b2_detector_period_n/checkpoints/fold_0/b2-model-best-epoch-10.tar" \
  --data_frac 1.0 \
  --dataset 'ViNDr' \
